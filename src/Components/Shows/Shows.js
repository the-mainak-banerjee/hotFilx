...
function Shows( {show}) {
  ...
  return (
    <div className='text-white w-[150px] sm:w-[200px] md:w-[250px] inline-block cursor-pointer relative hover:scale-110 mr-6'>
      <img 
        className='w-full h-auto block rounded-md' 
        src={`https://image.tmdb.org/t/p/w500/${show?.poster_path}`} 
        alt={show?.title || show?.name}
      />
      <div className='absolute top-0 left-0 w-full h-full flex items-start justify-center pl-2 flex-col opacity-0 hover:opacity-100 hover:bg-black/80 whitespace-normal'>
          <h2 
