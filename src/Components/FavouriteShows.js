...
function FavouriteShows() {
    ...
    const displayShowsCard = programs.map(item=> {
        return (
            <div 
                key={item.id}
    ...
    return(
        <section className='text-white p-5 md:px-16 md:pt-16'>
            ...
            {programs.length === 0 && <h4 className='relative text-xl md:mb-5 md:mt-0 md:pl-4 mt-20 pl-4'>Your WatchList Is Empty</h4>}
            {programs.length > 0 && <div className='relative flex items-center group'>
                <AiFillLeftCircle 
                    onClick= {slideLeft}
